...
import { ShieldCheckIcon, BookOpenIcon, MenuIcon } from "lucide-react"
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="left" className="w-72">
        ...
        <nav className="flex flex-col gap-1 px-4">
          {APP_NAV_ITEMS.map(({ href, label, icon: Icon, exact }) => {
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <nav className="hidden items-center gap-1 md:flex">
              {APP_NAV_ITEMS.map(({ href, label, icon, exact }) => (
                <NavLink
                  key={href}
                  href={href}
                  icon={icon}
                  active={isNavActive(pathname, href, exact)}
                >
                  {label}
                </NavLink>
              ))}
          ...
          <div className="flex items-center gap-1">
            <NavLink
              href="/docs"
              icon={BookOpenIcon}
              active={isNavActive(pathname, "/docs")}
            >
              Docs
            </NavLink>
            <ThemeToggle />
